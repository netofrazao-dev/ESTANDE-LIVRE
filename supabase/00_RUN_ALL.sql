...
-- Seguro rodar de novo — cada trecho usa "if not exists", "or replace",
-- "on conflict do nothing", blocos DO com captura de exceção pros enums,
-- e DROP explícito antes de recriar funções/views cuja assinatura mudou.
--
-- O que NÃO está aqui (passos manuais, fora do SQL Editor):
--   - Deploy das Edge Functions (notify-rentals, admin-create-reader)
--   - cron_setup.sql (depois da Edge Function deployada)
--   - Configurações no painel (Site URL, SMTP, Captcha) — ver README.md
--
-- ⚠️ migration_v6.sql semeia planos de preço com valores de EXEMPLO.
-- Revise em /admin/planos-de-preco antes de valer pra clientes de verdade.
-- ═══════════════════════════════════════════════════════════════════
...
-- ┌─────────────────────────────────────────────────────────────────────┐
-- │ 13) seed.sql (opcional — categorias e livros de exemplo)       │
-- └─────────────────────────────────────────────────────────────────────┘
